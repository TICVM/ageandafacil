...
import { Booking, Class, PhotoLocation, User, AppPermissions } from '@/lib/types';
...
export function BookingDetailsDialog({
  ...
  const currentStatus = (booking.status as StatusKey) || 'PENDING';
        ...
        <div className="grid grid-cols-1 md:grid-cols-2">
          <div className="p-8 space-y-6 border-r">
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button variant="ghost" size="sm" className="h-8 rounded-lg gap-1 text-primary">
                        <MoreHorizontal className="w-4 h-4" /> Alterar
                      </Button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent align="start" className="rounded-xl p-2 shadow-xl border-none min-w-[200px]">
                      {Object.entries(STATUS_CONFIG).map(([key, cfg]) => {
                        ...
                        return (
                          <DropdownMenuItem key={key} onSelect={() => onStatusUpdate(booking, key as StatusKey)} className="gap-2 rounded-lg py-2 cursor-pointer">
            ...
            <div>
              ...
              <p className="text-xl font-bold">{booking.teacherName}</p>
              <p className="text-sm font-medium text-primary">{classMap[booking.schoolClassId]?.name || '---'}</p>
            </div>

            <div className="flex flex-col gap-3">
              <div className="flex items-center gap-2 text-sm text-slate-600">
                <CalendarDays className="w-4 h-4 text-primary" />
                <span>{format(parseISO(`${booking.appointmentDate}T00:00:00`), 'dd/MM/yyyy')} às {booking.startTime}</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-slate-600">
                <MapPin className="w-4 h-4 text-primary" />
                <span>{locationMap[booking.photoLocationId]?.name || '---'} {booking.locationIdentifier ? `(${booking.locationIdentifier})` : ''}</span>
              </div>
            </div>
            ...
            <div className="bg-muted/30 p-6 rounded-2xl border border-dashed border-slate-300">
              ...
              <ScrollArea className="h-[100px]">
                <p className="text-sm italic text-slate-700 whitespace-pre-wrap">{booking.observations || "Sem notas."}</p>
              </ScrollArea>
            </div>
          </div>
          ...
          <div className="p-8 bg-slate-50">
            ...
            <ScrollArea className="h-[320px] pr-4">
              <div className="space-y-6 relative border-l-2 border-primary/20 pl-6 ml-2">
                {booking.history?.slice().reverse().map((e, idx) => (
        ...
        <DialogFooter className="p-6 border-t bg-white flex justify-between">
          <div className="flex gap-2">
            {userPerms?.canEditAppointments && (
              <Button variant="outline" onClick={() => onReschedule(booking)} className="rounded-xl border-primary text-primary hover:bg-primary/5">
                <Edit3 className="w-4 h-4 mr-2" /> Reagendar
              </Button>
            )}
            {userPerms?.canCancelAppointments && booking.status !== 'CANCELLED' && (
              <Button variant="outline" onClick={() => onStatusUpdate(booking, 'CANCELLED')} className="rounded-xl border-orange-200 text-orange-600 hover:bg-orange-50">
                <XCircle className="w-4 h-4 mr-2" /> Cancelar
              </Button>
            )}
            {userPerms?.canDeleteAppointments && (
              <Button variant="ghost" onClick={() => onDelete(booking.id)} className="rounded-xl text-destructive hover:bg-destructive/5">
                <Trash2 className="w-4 h-4 mr-2" /> Excluir
              </Button>
            )}
          </div>
          <Button onClick={onClose} className="rounded-xl px-10 h-11 font-bold">Fechar</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
