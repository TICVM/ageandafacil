...
import { getDaysInMonth, getMonthName, formatDateShort } from '@/lib/calendar/utils';
...
export function CalendarView({
  ...
  const getPublicationsByDate = (day: number) => {
  ...
  const isHoliday = (day: number): boolean => {
    const dateStr = `${year}-${String(month + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
    return holidays.some(h => h.date === dateStr || (h.recurring && h.date.endsWith(`-${String(month + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`)));
  };
